...
void Frame_Handler::show_histogramms(int h, int w)
{
    Gray_Histogramm  g_hist(&_src_image, h, w);
    BGR_Histogramm  bgr_hist(&_src_image, h, w);
    g_hist.calc_hist(h, w);
    bgr_hist.calc_hist(h, w);
    g_hist.draw_hist(h, w);
    bgr_hist.draw_hist(h, w);
    g_hist.show_hist();
    bgr_hist.show_hist();
}
